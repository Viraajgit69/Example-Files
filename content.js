...
(function () {
  ...
  const config = {
    ...
    generateRandomName: () => {
      const firstNames = ['𝘿𝘼𝙉𝙂𝙀𝙍'];
      const lastNames = ['𝘿𝘼𝘿𝘿𝙔'];
  ...
  class StripeHelper {
    ...
    fillFields() {
      ...
      Object.entries(config.fields).forEach(([key, selector]) => {
        ...
        if (field) {
          field.value = address[key] || '𝘿𝘼𝙉𝙂𝙀𝙍';
    ...
    showEmailDialog() {
      ...
      input.placeholder = 'Enter Email (e.g., fuckyou69@gmail.com)';
    ...
    const replaceText = () => {
      ...
      while (node = walker.nextNode()) {
        if (node.nodeValue.includes('Powered by')) {
          node.nodeValue = node.nodeValue.replace(/Powered by/g, "𝘿𝘼𝙉𝙂𝙀𝙍");
        }
      }
    };
